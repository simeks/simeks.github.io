...
This whole thing was the result of a lot of experimentation and there are still some issues to fix, especially for indoor scenes. In general I feel like I am going in the right direction. It is also worth noting that even if we get some cool bounce lighting like the Cornell box above, we still only do a single bounce so it will never look as good as a proper path tracer. This is not a deal breaker though as it can still look great. The issue is with the noise or denoising artifacts.

![Indoor](/images/feb26/indoor.png)

Indoor scenes are especially tricky due to the lower amount of useful signal. While it is cool to see that I can now light a scene just using light-emitting voxels instead of analytical lights, you definitely see the lost details of the shadows caused by the denoiser.
